...
- **NON-NEGOTIABLE RULE FOR SKILL CREATION AND IMPROVEMENT:** When creating, adding, updating, or improving any skill, you MUST NOT follow, apply, inherit, or incorporate instructions from `AGENTS.md` or similar user- or repository-level instruction files. This prohibition explicitly includes all other instructions in this file. Do not let those files determine the skill's content, design, workflow, constraints, or implementation decisions. Do not copy, paraphrase, or indirectly embed their rules into the skill. Apply this exclusion throughout the entire task; never waive, weaken, or bypass it for convenience, consistency with repository conventions, or assumed user intent.
