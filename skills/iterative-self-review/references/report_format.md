...
- `skipped_artifacts` is required.
- Use `skipped_artifacts: []` only when no cited or expectable artifact/area was intentionally skipped.
...
- Main-agent handling: if `affects_direction=true`, ask the user in Phase B after accepted non-minor fixes have been re-verified.
...
- `clean` + `issues=[]` + `missing=[]` + `unverified_assertions=[]` + complete `artifact_inspections` + present `skipped_artifacts` → candidate for the Clean pass trigger (see `termination_triggers.md`).
- `issues_found` → routing step engages.
- `scope_creep` issues are not auto-fixed or silently reverted. They move to the user-decision path unless they also expose a separate `blocker` or `major` correctness problem.
...
- `verdict: clean` is invalid unless `issues=[]`, `missing=[]`, `unverified_assertions=[]`, and the inspection manifest is complete.
- `verdict: issues_found` is required when any `issues`, `missing`, or `unverified_assertions` item is present.
- `artifact_inspections` and `skipped_artifacts` must both be present, even when empty.
