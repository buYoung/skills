...
description: >
  ...
  caveman **full** mode for ~75% token compression while keeping technical
  accuracy. Eight required sections keyed to Conventional Commits types so
  downstream coding agents switch behavior (refactor → preserve, fix →
  reproduce first, perf → measure first). Briefset mode emits a parent
  ...
  reporting stay in normal prose — caveman applies to the saved file
  only.
---
...
short synonyms — to compress the document ~75% while keeping every
technical fact intact. Other intensity levels (`lite`, `ultra`,
`wenyan-*`) are out of scope; do not switch.
...
- Short synonyms preferred (`big` over `extensive`, `fix` over
