...
`task-brief-creator-caveman` defaults to producing **one brief per invocation**.
...
The template skeleton below uses normal prose for readability; in the saved file, every prose bullet is rewritten in caveman full mode per `caveman-style.md`.
Headers, the title line, child paths, and checklist markers stay verbatim.
See example 04 for a converted parent.
...
- Children also follow `references/caveman-style.md` — body prose in caveman full mode, Auto-Clarity carve-outs preserved.
  The same rules that apply to a single-brief saved file apply to each child file.
...
The caveman-parity check applies to children but not to the parent's execution-management sections (the parent is allowed to be short-prose by nature).
