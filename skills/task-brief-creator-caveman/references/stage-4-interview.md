...
`task-brief-creator-caveman` closes residual ambiguity through one policy: first gather enough codebase context in Stage 3, then present every remaining user-owned decision in a Markdown decision table.
...
After termination, proceed to Stage 5 (save + structural validate) and Stage 5.5 (content-level self-check) exactly as documented in `SKILL.md`.
