...
| [05-stage-4-walkthrough.md](05-stage-4-walkthrough.md) | Korean refactor note | `refactor` | Focused Stage 4 example; how codebase probes remove technical questions, the remaining scope decisions become a user decision table, and the Stage 5.6 cold-pickup gate fires on a `refactor` type with the caveman-only `over_terse_bullets` report field. Behavior Contract precision still survives caveman. |
