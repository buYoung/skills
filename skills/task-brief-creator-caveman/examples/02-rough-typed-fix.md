...
## Reproduction
- Steps: open `/login` on iOS Safari 17, then enter known-valid account whose password contain `@` (test account `qa+iossafari@example.com` / `foo@bar`), then tap "Log in".
...
### Out of Scope
- Refactor or restructure validation chain beyond offending step.
- Change `/api/login` request or response contract.
- Review other characters that may have similar issues — separate brief if found.
...
- [ ] Manual verification on iOS Safari 17 with password `foo@bar` result in successful login.
