...
3. Translate confirmed qualitative feedback into invariant, variable, conditional, prohibited, and verification decisions. Read [design-rule-contract.md](design-rule-contract.md) when drafting reusable rules: connect their rationale, application conditions, supported implementation or assets, fixed mechanics versus permitted variation, and observable checks. Locate each decision's canonical owning file before drafting cross-references.
...
5. Create or update only the selected prebuilt owners supported by approved decisions, verified facts, or necessary unresolved implementation details. Add only explicitly required platform or storefront files, observing the decision table's partial-authoring boundary. For a reusable template that governs placement, complete its asset owner's source-to-application map before drafting composition examples.
...
7. Check names, values, links, decision classes, and ownership across the set. Check material rules for missing rationale, application conditions, implementation or asset connections, and conflicts with approved guidance using the common rule contract. Check example invocation and return contracts against their actual sources. Trace template regions, coordinate transforms, and boundary roles to the consuming placement rules when they govern composition. For recurring corrections, connect comparable follow-up observations and unresolved recurrence to the existing operating owner. For an existing set, also compare against the original for accidental loss, changed meaning, and altered exact text.
