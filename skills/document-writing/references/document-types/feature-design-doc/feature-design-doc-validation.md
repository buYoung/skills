...
2. Compare material claims in that text with the inputs and note using the note's decision classes and alternative-behavior criterion. Pay particular attention to exact values, positive and negative policies, preconditions, exception branches, failure outcomes, and state changes. Identify statements that gained certainty or introduced a choice without support, including explanatory inferences promoted to policy or historical rationale. For an unresolved branch, compare its scope with general rules elsewhere so those rules do not silently settle its outcome. Check in the reverse direction that supplied decisions were not lost or weakened. Do not use the new draft itself as evidence for its additions.
3. Trace every current exact rule in scope to its one owning section and inspect other occurrences, including paraphrases, using the [decision ownership rules](feature-design-doc-section-responsibilities.md#decision-ownership-and-semantic-deduplication). Compare each policy owner's applicability conditions and response with those stated or referenced in consuming sections. Distinguish a linked explanation of a stated local context from a redefinition of policy scope. Flag unsupported expansion or restriction, including a response extended to a broader concept through generalized or omitted conditions; require a separate request or evidence-backed decision for a scope change. Retain the authoritative definition and supported rationale at the owner; allow concise linked summaries, section-specific effects, and user outcomes elsewhere. Flag independent redefinitions or divergence, not repeated words or values alone. Check that consolidation preserves meaning and references resolve. Preserve historical and protected exact text under the section-responsibility rules.
4. Apply the remaining model-responsibility checks above, including implementation leakage and metadata coverage. In authoring modes, correct unsupported additions introduced by the work, including policy-scope changes, and duplicate current definitions within scope. Leave material choices within the actual requested scope explicitly open; omit out-of-scope hypothetical cases rather than adding required questions. For pre-existing conflicting or unverified decisions, report the gap and preserve their authority/history unless the request and evidence authorize changing them. In review-only modes, report findings and proposed corrections without writing any files or metadata.
