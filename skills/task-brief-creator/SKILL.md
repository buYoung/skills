...
1. Report the path, a one-line summary (work type + title), the structural validator result, **and the Stage 5.5 self-check result**.
   ...
   **English (validator + self-check passed):**
   > Saved — `docs/briefs/2026-04-23-feat-dark-mode-settings.md` (`feat`: Dark mode toggle in Settings; structural validation passed; content self-check passed — N input concerns covered).
   > Open it and let me know if anything needs editing.

   **Korean (validator + self-check passed):**
   > 저장 완료 — `docs/briefs/2026-04-23-feat-dark-mode-settings.md` (`feat`: Dark mode toggle in Settings; 구조 검증 통과; 내용 자체 검증 통과 — 입력 항목 N개 모두 매핑됨).
