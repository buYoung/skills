...
- Phase 1 — blind reconstruction: read the brief (and parent) first and write `execution_reconstruction` completely before opening the input file.
...
- Every `intent_deviations[*]`, `ask_backs[*]`, and `missing_concerns[*]` carries direct-quote evidence. Paraphrases are not accepted; if no quote applies, drop the item.
...
- If the original input included a source-of-truth checklist, TODO file, review rubric, or audit document, a clean verdict requires item-level coverage: each item is represented, explicitly deferred / out of scope, or preserved as an Open Question. A briefset child counts only the items the parent assigns to it. Representative theme coverage is not clean.
- `verdict: clean` is valid only when `intent_deviations`, `ask_backs`, and `missing_concerns` are all empty and `execution_reconstruction` is complete.
...
- Briefset, all clean — `cold-pickup: parent + N/N children clean`.
