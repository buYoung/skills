...
Each child runs the standard 5-item self-check from `SKILL.md`
Stage 5.5. If any child fails the input-coverage or section-depth
items, fix the child in place, re-run the briefset structural
validator, and re-run the child's self-check. Do not skip the child
self-check on the assumption "the parent covers it" — children are
independently executable, so they are independently completeness-checked.
...
disk via `Edit`; do not re-render the briefs into chat.
